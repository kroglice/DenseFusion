...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seed_real = random.choice(self.real)
        seed_syn_last = None
        idx_last = None
        ...
        head_file_prefix = file_prefix[:-6]
        tail_file_prefix = file_prefix[-6:]
        frame_idx = int(tail_file_prefix)
        clouds, chooses, imgs_masked, targets, models_points, idxes = [], [], [], [], [], []
        for i in range(self.timesteps):
            file_prefix = head_file_prefix + '{0:06d}'.format(frame_idx)
            frame_idx += 1
            ...
            mask_back = ma.getmaskarray(ma.masked_equal(label, 0))  # label here

            add_front = False
            if self.add_noise:
                for k in range(5):
                    seed_syn = random.choice(self.syn) if seed_syn_last is None else seed_syn_last  # TODO check this
                    front = np.array(self.trancolor(Image.open('{0}/{1}-color.png'.format(self.root, seed_syn)).convert("RGB")))
                    front = np.transpose(front, (2, 0, 1))
                    f_label = np.array(Image.open('{0}/{1}-label.png'.format(self.root, seed_syn)))
                    ...
                    t_label = label * mask_front                    # label here so mask_front should be the same for all successive frames
                    if len(t_label.nonzero()[0]) > 1000:
                        label = t_label
                        add_front = True
                        seed_syn_last = seed_syn
                        break

            obj = meta['cls_indexes'].flatten().astype(np.int32)

            while 1:
                idx = np.random.randint(0, len(obj)) if idx_last is None else idx_last                           # TODO idx should be the same too
                mask_depth = ma.getmaskarray(ma.masked_not_equal(depth, 0))     # depth here
                mask_label = ma.getmaskarray(ma.masked_equal(label, obj[idx]))  # label of obj[idx] here
                mask = mask_label * mask_depth
                if len(mask.nonzero()[0]) > self.minimum_num_pt:
                    idx_last = idx
                    break
            print(i, seed_syn_last, idx_last)
            if self.add_noise:
            ...
            if self.list[index][:8] == 'data_syn':
                # seed_real = random.choice(self.real)
                back = np.array(self.trancolor(Image.open('{0}/{1}-color.png'.format(self.root, seed_real)).convert("RGB")))
                back = np.transpose(back, (2, 0, 1))[:, rmin:rmax, cmin:cmax]
                img_masked = back * mask_back[rmin:rmax, cmin:cmax] + img       # label here
            else:
                img_masked = img
